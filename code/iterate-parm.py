...
def set_aniso_dir():
    ...
    Process.Mesh__Selection__Action__Clear_Selection('All')
    Process.Mesh__Selection__Action__Select_All('Faces')
    Process.Model__CCF__06_Set_Ansio_Dir('Linear Triangle', 'Triangle Element', 'E2', '0.0 1.0 0.0', const.anisoDir, '1e-6')
...
rod_width = [1.0, 3.0, 5.0, 7.0, 9.0]
make_dir(const.out_dir)

###############################
## Iterating through a parameter
###############################
for n in range(0,len(rod_width)):
    
    out_dir = const.out_dir + str(rod_width[n])  
    make_dir(out_dir)
    load_initial_mesh(const.initial_mesh)
    write_csv_save_mesh(0, 0, 0, out_dir)
    ref_cfg(rod_width[n], const.wall_thickness)
    set_stress_strain()
    set_aniso_dir()
    set_boundary_conds()
